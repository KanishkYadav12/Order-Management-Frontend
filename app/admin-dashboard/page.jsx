...
export default function UserList() {
  ...
  const { loading: approveLoading, handleApproveOwner } =
    useApproveOwner(setApprovingItemId);

  const { loading: extendOwnerMembershipLoading, handleExtendOwnerMembership } =
    useExtendOwnerMembership(setSelectedUserId);
        ...
        {loading ? (
          ...
          owners.map((user) => (
            <TableRow key={user._id}>
              ...
              <TableCell>
                {getExpiryTimeInWords(user.membershipExpires) || "N/A"}
              </TableCell>
              <TableCell>
                <div className="flex space-x-2">
                  ...
                  >
                    {approvingItemId == user._id.toString() ? (
                      <Spinner size="sm" />
                    ) : user.isApproved ? (
                      "Reject Owner"
                    ) : (
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4">
                        <Input
                          ...
                          value={extendDays || 0}
                          onChange={(e) =>
                            setExtendDays(parseInt(e.target.value, 10))
                          }
                        />
                      </div>
                      <DialogFooter>
